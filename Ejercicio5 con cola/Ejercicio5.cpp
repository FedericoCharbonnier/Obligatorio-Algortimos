...
public:
    Asociacion() {}
    Asociacion(int unaClave, int unIndice, int unDest) : clave(unaClave), index(unIndice), dest(unDest) {}
...
public:
    ...
    void dijkstra(int posO){
        ...
        while (aux != NULL)
        {
            Asociacion a(aux->getCosto(), posO+1, aux->getDestino());
            cola->insertar(a);
            dist[aux->getDestino()-1] = aux->getCosto(); 
            aux = aux->getSig();
        }
        
        while(!cola->esVacio()){
            Asociacion ayuda = cola->pop();
            if (ayuda.getClave() + dist[ayuda.getIndex()-1] <= dist[ayuda.getDest()-1]){
                dist[ayuda.getDest()-1] = ayuda.getClave() + dist[ayuda.getIndex()-1];
            }
            if (!vis[ayuda.getDest()-1]){
                aux = listaAdy[ayuda.getDest()-1];
                while (aux != NULL){
                    if (aux->getCosto() + dist[ayuda.getDest()-1] < dist[aux->getDestino()-1]) {
                        dist[aux->getDestino()-1] = aux->getCosto() + dist[ayuda.getDest()-1];
                    }
                    Asociacion nuevo(aux->getCosto(), ayuda.getDest(), aux->getDestino());
                    cola->insertar(nuevo);
                    aux = aux->getSig();
                }
                vis[ayuda.getDest()-1] = true;
            }
        }
